...
TARGET_ISO=${2:-"`pwd`/ubuntu-16.10-netboot-amd64-unattended-$ROOT_PASSWORD.iso"}
...
cd "$TMP_DISC_DIR"
rm -r '[BOOT]'
mkisofs -r -V "ubuntu 16.10 netboot unattended" -cache-inodes -J -l -b isolinux.bin -c boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -input-charset utf-8 -o "$TARGET_ISO" ./
